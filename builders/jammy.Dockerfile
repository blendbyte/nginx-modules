...
RUN FP=573BFD6B3D8FBC641079A6ABABF5BD827BD9BF62 && \
    ...
    echo "deb [signed-by=/etc/apt/keyrings/nginx.gpg] https://nginx.org/packages/ubuntu jammy nginx" \
        > /etc/apt/sources.list.d/nginx.list && \
    echo "deb-src [signed-by=/etc/apt/keyrings/nginx.gpg] https://nginx.org/packages/ubuntu jammy nginx" \
        >> /etc/apt/sources.list.d/nginx.list && \
    apt-get update
